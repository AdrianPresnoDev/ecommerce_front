...
export const createCheckout = (data) =>
  request('/api/v1/checkout', { method: 'POST', body: JSON.stringify(data) });

// ─── About / Site Settings (público) ─────────────────────────────────────────
export const getAbout = () => request('/api/v1/about');

// ─── About / Site Settings (admin) ───────────────────────────────────────────
export const adminGetAbout = (apiKey) =>
  adminRequest('/admin/api/settings/about', {}, apiKey);

export const adminUpdateAbout = (apiKey, data) =>
  adminRequest('/admin/api/settings/about', { method: 'PATCH', body: JSON.stringify(data) }, apiKey);

export const adminGetAboutImageUploadUrl = (apiKey, contentType) =>
  adminRequest('/admin/api/settings/about/images/presign', {
    method: 'POST', body: JSON.stringify({ contentType }),
  }, apiKey);
